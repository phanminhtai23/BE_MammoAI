...
async def create_user_session(
    user_id: str, auth_provider: str = "local", session_hours: int = 1
):
    """
...
async def end_user_session(session_id: str):
    ...
    try:
        await users_session_collection.update_one(
            ...
            {"$set": {"logout_at": datetime.now(timezone.utc), "is_active": False}},
        )
        return True
    except Exception as e:
...
async def verify_code(data: VerifyCode):
    ...
    verification_code = await verification_codes_collection.find_one(
        {"email": data.email}
    )

    if not verification_code:
        raise HTTPException(
            status_code=404, detail="Email chưa đăng ký hoặc đã xác thực tài khoản"
        )
    ...
    await users_collection.update_one(
        {"email": data.email}, {"$set": {"confirmed": True}}
    )
...
async def login(user: LoginRequest):
    ...
    if not db_user.get("confirmed", False):
        raise HTTPException(status_code=403, detail="Tài khoản chưa xác thực")
...
async def forgot_password(data: SendVerificationCode):
    ...
    expires_at_1 = datetime.now(timezone.utc) + timedelta(
        minutes=VERIFICATION_CODE_EXPIRE_MINUTES
    )
    ...
    mail_service.send_forgot_password_email(
        data.email, token, expires_at_vn, reset_password_url
    )
...
async def check_token(token: VerifyResetToken):
    ...
    return {"status": 200, "message": "Token còn hạn", "payload": payload}
...
async def reset_password(data: ResetPassword):
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail="Lỗi khi đặt lại mật khẩu")
...
async def google_login(req: GoogleLoginRequest):
    try:
        ...
        db_user = await users_collection.find_one(
            {"provider_id": response["id"], "auth_provider": "google"}
        )
...
async def facebook_login(req: FacebookLoginRequest):
    try:
        ...
        # print("response:", response)
        db_user = await users_collection.find_one(
            {"provider_id": response["id"], "auth_provider": "facebook"}
        )
...
async def google_register(req: GoogleLoginRequest):
    try:
        ...
        if existing_user:
            raise HTTPException(
                status_code=400, detail="Tài khoản Google đã được đăng ký"
            )
        else:
            user_data = {
                "id": response["id"],
...
async def facebook_register(req: FacebookLoginRequest):
    try:
        ...
        if existing_user:
            raise HTTPException(
                status_code=400, detail="Tài khoản facebook đã được đăng ký"
            )
        else:
            user_data = {
                "id": response["id"],
