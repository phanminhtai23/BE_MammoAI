...
async def predict(request: PredictRequest, user = Depends(verify_token)):
    try:
        ...
        predicted_class_name = bi_rads_names.get(max_class_index, f"Class {max_class_index}")
...
async def create_model(
    ...
    try:
        ...
        existing_model = await models_collection.find_one({"name": model_data.name, "version": model_data.version})
        if existing_model:
            raise HTTPException(status_code=400, detail="Model với tên và phiên bản này đã tồn tại")
