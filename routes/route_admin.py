...
async def create_dataset_download(
    ...
    if train_percent + val_percent + test_percent != 100:
        raise HTTPException(
            status_code=400,
            detail="Tổng phần trăm phải bằng 100%"
        )
    try:
        ...
        if not predictions:
            raise HTTPException(
                status_code=404,
                detail="Không có dữ liệu predictions để tạo dataset"
            )
        class_groups = {}
