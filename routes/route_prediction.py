...
async def get_predictions_by_doctor_id(
    doctor_id: str, page: int = 1, limit: int = 20, user: dict = Depends(verify_token)
):
    try:
        skip = (page - 1) * limit
        cursor = (
            predictions_collection.find({"doctor_id": doctor_id})
            .sort("created_at", -1)
            .skip(skip)
            .limit(limit)
        )
...
async def get_all_predictions(
    ...
    try:
        ...
        skip = (page - 1) * limit
        predictions_cursor = (
            predictions_collection.find(filter_query)
            .sort("created_at", -1)
            .skip(skip)
            .limit(limit)
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi khi lấy danh sách predictions: {str(e)}"
        )
...
async def get_prediction_by_id(prediction_id: str, user: dict = Depends(verify_token)):
    ...
    try:
        ...
        if not prediction:
            raise HTTPException(
                status_code=404, detail="Không tìm thấy prediction với ID này"
            )
        ...
        raise HTTPException(status_code=500, detail=f"Lỗi khi lấy prediction: {str(e)}")


@router.delete("/{prediction_id}")
async def delete_prediction(
    prediction_id: str, file_key: str, user: dict = Depends(verify_token)
):
    """
    ...
    try:
        ...
        existing_prediction = await predictions_collection.find_one(
            {"id": prediction_id}
        )
        ...
        return JSONResponse(
            status_code=200,
            content={"success": True, "message": "Xóa prediction thành công"},
        )

    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Lỗi khi xóa prediction: {str(e)}")


@router.get("/statistics/daily")
async def get_daily_prediction_stats(
    days: int = 30, doctor_id: Optional[str] = None, user: dict = Depends(verify_token)
):
    """
    ...
    try:
        ...
        end_date_vietnam = vietnam_now.replace(
            hour=23, minute=59, second=59, microsecond=999999
        )
        start_date_vietnam = (vietnam_now - timedelta(days=days)).replace(
        ...
        # Tạo filter query
        filter_query = {"created_at": {"$gte": start_date_utc, "$lte": end_date_utc}}

        if doctor_id:
            filter_query["doctor_id"] = doctor_id

        # Lấy predictions trong khoảng thời gian
        predictions = await predictions_collection.find(filter_query).to_list(
            length=None
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi khi lấy thống kê daily predictions: {str(e)}"
        )


@router.get("/statistics/class-distribution")
async def get_class_distribution_stats(
    doctor_id: Optional[str] = None, user: dict = Depends(verify_token)
):
    """
    ...
    try:
        ...
        return JSONResponse(
            ...
            content={
                "success": True,
                "data": {"class_stats": class_stats, "total_images": total_images},
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi khi lấy thống kê class distribution: {str(e)}"
        )


@router.get("/statistics/admin-daily")
async def get_admin_daily_stats(
    days: int = 30, user: dict = Depends(verify_admin_token)
):
    """
    ...
    try:
        ...
        vietnam_now = datetime.utcnow() + timedelta(hours=7)
        end_date = vietnam_now.replace(
            hour=23, minute=59, second=59, microsecond=999999
        )
        start_date = (vietnam_now - timedelta(days=days)).replace(
            hour=0, minute=0, second=0, microsecond=0
        )
...
async def get_admin_class_distribution_stats(user: dict = Depends(verify_admin_token)):
    """
...
async def get_admin_average_confidence(user: dict = Depends(verify_admin_token)):
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Lỗi khi lấy độ tin cậy trung bình admin: {str(e)}"
        )
