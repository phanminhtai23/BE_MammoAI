...
class ModelAI:
    ...
    def _create_resnet_with_dropout(self, num_classes=6, is_gray=False, dropout_prob=0.4):
        """
        ...
        model = ResNetWithDropout(num_classes=num_classes, is_gray=is_gray, dropout_prob=dropout_prob)
        print("✅ Custom ResNetWithDropout created successfully")
        return model
    

    
    def _create_inception_v3_with_dropout(self, num_classes=6, is_gray=False, dropout_prob=0.4):
        """
        ...
        class InceptionV3WithDropout(nn.Module):
            def __init__(self, num_classes=6, is_gray=False, dropout_prob=0.2):
                ...
                self.model = models.inception_v3(
                    weights=models.Inception_V3_Weights.DEFAULT, aux_logits=True, transform_input=False)
                ...
                if self.model.AuxLogits is not None:
                    ...
                    self.model.AuxLogits.fc = nn.Sequential(
                        nn.Dropout(p=dropout_prob),
                        nn.Linear(num_aux_ftrs, num_classes)
                    )
                else:
                    print("Warning: AuxLogits is None, auxiliary classifier not replaced.")
        ...
        model = InceptionV3WithDropout(num_classes=num_classes, is_gray=is_gray, dropout_prob=dropout_prob)
        print("✅ Custom InceptionV3WithDropout created successfully")
        return model
    
    def _create_mobilenet_v2_with_dropout(self, num_classes=6, is_gray=False, dropout_prob=0.2):
        """
        ...
        model = MobileNetV2WithDropout(num_classes=num_classes, is_gray=is_gray, dropout_prob=dropout_prob)
    ...
    async def load_model(self, current_model: dict) -> bool:
        ...
        try:
            if not self.model_path.exists():
                ...
                result_download = await self._download_model(current_model['model_url'], self.model_path)
                ...
                async with aiohttp.ClientSession(
                    ...
                    async with session.get(model_url) as response:
                        if response.status == 200:
                            ...
                            print(f"✅ Downloaded: {save_path.name} ({file_size:.1f} MB)")
                            return True
                        else:
    ...
    async def predict_image(self, image_url: str) -> Tuple[Optional[int], Optional[str], Optional[float], Optional[List[float]]]:
        """
        Dự đoán ảnh từ URL
    ...
    async def get_prediction_details(self, image_url: str) -> Dict:
        ...
        """
        predicted_id, predicted_class, predicted_prob, all_probs = await self.predict_image(image_url)
